#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)demorandom.c 20.15 93/06/28 SMI";	/* from Lucasfilm Ltd.,
							 * 1982 */
#endif
#endif
...
/*
 * Sun Circles by Jeffrey Mogul stolen from: Hackmem Minksy? draw a circle of
 * radius "radius" centered at cx, cy pv is either 1 or 0 for white or black
 * r should be something like 4
 */

/*
 * clearscreen(){ drasterop(GXclear, 0, 0, NULL, 1024, 1024); } setscreen(){
 * drasterop(GXset, 0, 0, NULL, 1024, 1024); } circ(radius, r, cx, cy, pv)
 * register int r, cx, cy; { register origin(cx, cy); register int x,y;
 * register int i; register int ilast; register xmax=screen.w/2; register
 * ymax=screen.h/2; lockscreen(); width(1); setmode(pv); x=radius; y=0;
 * ilast=radius<<(r-3); for(i=0;i<ilast;i++){ if(-xmax<=x && x<xmax &&
 * -ymax<=y && y<ymax) point(x, y); x+=y>>r; y-=x>>r; } unlockscreen(); }
 * combox(op, x, y, xs, ys){ if(x<0){ xs+=x; x=0; } if(y<0){ ys+=y; y=0; }
 * if(x<WIDTH && y<HEIGHT){ if(x+xs>=WIDTH) xs=WIDTH-1-x; if(y+ys>=HEIGHT)
 * ys=HEIGHT-y; if(xs>0 && ys>0) drasterop(op, x, y, NULL, xs, ys); } }
 */
