#ifndef	lint
#ifdef sccs
static char     sccsid[] = "@(#)ndetpevent.c 20.14 93/06/28 Copyr 1985 Sun Micro";
#endif
#endif
 ...
 * possible. There are two ways that this could be done correctly: 1) Note
 * when it is not safe and redirect events to delayed deliver if they come
 * through during unsafe times. 2) Note when it is safe and and redirect
 * events to delayed deliver if they come through during unsafe times. We
 * want to choose the lowest overhead approach.
 * 
 * It is not safe during an interrupt or while another event is being processed
 * by the client.  We would like to have the prioritizer get a look at all
 * SAFE events, but this is not required.  After all, the prioritizer doesn't
 * look at destroy notifications.  If someone wants to look at all events,
 * then they had better interpose themselves in a client's event path.
 * 
    ...
    Notify_release  release_func;
{
    ...
    /*
     * If tried to send immediate, there are two circumstances when we want
     * to try to send safe.  One is when an immediate func could not be
     * found.  The other is when the event was ignored.
     * 
     * If tried to send safe, you only try to send immediately if there was no
     * safe condition.
     */
	...
	if ((return_code = ndet_p_event(nclient, event,
	    ...
	    NOTIFY_NO_CONDITION)
	    /*
	     * Set notify_errno again because NOTIFY_NO_CONDITION was
	     * originally just set as a warning in ndet_p_event.
	     */
...
Now:
	/*
	 * Remember nclient because client->nclient may not be valid when
	 * return from func.
	 */
	...
	*rc = func(nclient_now, event, arg, now_type);
	...
	/* Also send it now if signal handlers are deaf */
	if (!(client->flags & NCLT_EVENT_PROCESSING) &&
	    (!NTFY_IN_INTERRUPT || NTFY_DEAF_INTERRUPT)) {
